...
def polylabel(polygon, precision=1.0, debug=False):
    ...
    for x in range(minX, maxX - 1, cellSize):
        for y in range(minY, maxY - 1, cellSize):
...
def pointToPolygonDist(x, y, polygon):
    ...
    for ring in polygon:
        for a, b in zip(ring, rotate(ring)):
...
def getCentroidCell(polygon):
    ...
    points = polygon[0]
    for a, b in zip(points, rotate(points)):
        f = a[0] * b[1] - b[0] * a[1]
        x += (a[0] + b[0]) * f
        y += (a[1] + b[1]) * f
        area += f * 3
